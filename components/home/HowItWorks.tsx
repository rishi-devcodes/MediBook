      ...
      <Container>
        ...
        <div className="grid gap-8 sm:grid-cols-3">
          {STEPS.map((step, i) => (
            ...
            >
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-primary-50 text-primary-500 transition-all duration-200 group-hover:bg-primary-500 group-hover:text-white">
                <step.icon className="h-6 w-6" />
              </div>

              <span className="text-xs font-semibold tracking-widest text-primary-500">
                {step.number}
              </span>
              ...
              {i < STEPS.length - 1 && (
                <span
                  aria-hidden
                  className="absolute right-[-20px] top-7 hidden h-px w-10 bg-border sm:block"
                />
              )}
            </div>
          ))}
